...
Eval tasks are opt-in and are not created by `mda init`. Author Harbor tasks directly under `evals/tasks/`, or run `mda evals init <name>` to create a minimal shortcut task under `mda_evals/`.
...
| Subcommand | Use |
| --- | --- |
| `mda evals init <name>` | Create `mda_evals/<name>/` with an instruction and a language-native test. Run this command from the project root. |
...
| Flag | Use |
| --- | --- |
| `--task <name>` | Refresh one task from `mda_evals/`. Repeat to select multiple tasks. If omitted, every shortcut task is refreshed. Other tasks under `evals/tasks/` are preserved. |
...
For direct Harbor authoring, the MDA shortcut, credentials, and running trials, see [Evals](/langsmith/javascript/managed-deep-agents-evals).
